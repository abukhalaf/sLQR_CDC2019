...
%   Off-policy learning of a Minimum Inervention policy.
%
%   Does not use Experience Replay.
%
%   Used to compare (no plots included) with Figure 4 in:
%
...
for i = 0 : (nTrajectorySamples-1)
    ...
    if j == nTrajectorySamplesPerPolicy
        Qc = Q + Ch'*Kh'*M*Kh*Ch;
        P_LYAP = lyap((A+B*Kh*Ch+B*K_LYAP)',Qc + K_LYAP'*R*K_LYAP); display(P_LYAP);
...
F = Ki;
[t,x]= ode45(@(t,x) AxBuIntervene(t,x,A,B,Ch,Kh,Ki), t(end) + [0 3], x(end,:)');
...
title('Off-Policy Learning of a Minimum Intervention Policy','FontSize',16);xlabel('Time (sec)','FontSize',16);ylabel('State Variables','FontSize',16);
legend({'$\tilde{V_1}(t)$','$\tilde{S}(t)$','$\tilde{V_2}(t)$'},'FontSize',20,'Interpreter','latex')
...
legend({'$u_h(t)$','$u_a(t)$'},'FontSize',20,'Interpreter','latex')
...
function x_vdot = AxBuIntervene(~,x,A,B,Ch,Kh,Ki)
[n,~]=size(B);
